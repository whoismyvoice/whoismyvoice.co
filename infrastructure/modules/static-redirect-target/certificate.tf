 ...
 *
resource "aws_route53_record" "redirect_cert_validation" {
  count = "${length(aws_acm_certificate.redirect_cert.domain_validation_options)}"
  name = "${lookup(element(aws_acm_certificate.redirect_cert.domain_validation_options, count.index), "resource_record_name")}"
  type = "${lookup(element(aws_acm_certificate.redirect_cert.domain_validation_options, count.index), "resource_record_type")}"
  zone_id = "${var.dns_zone_id}"
  records = [
    "${lookup(element(aws_acm_certificate.redirect_cert.domain_validation_options, count.index), "resource_record_value")}",
  ]
  ttl = 60
}
*/
