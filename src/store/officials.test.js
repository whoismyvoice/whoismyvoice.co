...
it('adds an official to dictionary', () => {
  const state = officials(undefined, receiveOfficialsAll([
...
it('adds an id of official to list', () => {
  const state = officials(undefined, receiveOfficials([
    { name: 'John Smith', },
  ]));
  const { ids, } = state;
...
it('merges data from receiveOfficials and receiveOfficialsAll', () => {
  const initialState = officials(undefined, receiveOfficials([
    { name: 'John Smith', photoUrl: '1', },
    { name: 'John Smith Jr.', photoUrl: '2', },
  ]));
...
describe('existing state', () => {
  ...
  beforeEach(() => {
    initialState = officials(undefined, receiveOfficials([
      { name: 'John Smith', photoUrl: '1', },
      { name: 'John Smith Jr.', photoUrl: '2', },
    ]));
  });
