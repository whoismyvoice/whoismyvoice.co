import { Record as ContributionRecord } from './ContributionT';
...
export class Legislator {
   ...
   * @param {Record} record that matches the `PropType` defined for
   *    `Legislator` instances.
   * @returns the identifier used for legislator records for the given `record`.
   */
  static getIdentifier(record: Record): Identifier {
